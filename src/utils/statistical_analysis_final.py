"""
Add statistical analysis to all experimental results - FINAL CORRECTED VERSION.
"""
import pandas as pd
import numpy as np
...
def add_statistical_analysis():
    ...
    if grad_file.exists():
        ...
        print(f"✓ Phase 2 columns: {df.columns.tolist()}")
    ...
    # 2. Phase 3: Compilation results - FIXED
    comp_file = Path("experiments/thrust2/final_adjusted/table2_adjusted.csv")
    if comp_file.exists():
        df = pd.read_csv(comp_file)
        print(f"✓ Phase 3 columns: {df.columns.tolist()}")
        print(f"✓ Sample data:\n{df.head()}")
        
        # Use the existing Gate Reduction (%) column
        if 'Gate Reduction (%)' in df.columns:
            reduction_col = 'Gate Reduction (%)'
        elif 'Gate Reduction' in df.columns:
            reduction_col = 'Gate Reduction'
        else:
            # Calculate from available columns
            if 'Original Gates' in df.columns and 'Optimized Gates' in df.columns:
                df['Gate Reduction (%)'] = (df['Original Gates'] - df['Optimized Gates']) / df['Original Gates'] * 100
                reduction_col = 'Gate Reduction (%)'
            else:
                print("⚠️ Could not find gate reduction data")
                return False
    ...
    if qec_file.exists():
        ...
        print(f"✓ Phase 4 columns: {df.columns.tolist()}")
