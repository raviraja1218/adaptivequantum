"""
Add statistical analysis to all experimental results - FIXED VERSION.
"""
import pandas as pd
import numpy as np
...
def add_statistical_analysis():
    ...
    if comp_file.exists():
        ...
        # Check which column contains reduction data
        if 'Reduction' in df.columns:
            reduction_col = 'Reduction'
        elif 'reduction' in df.columns:
            reduction_col = 'reduction'
        else:
            # Calculate reduction from IBM Gates and Our Gates
            df['Reduction'] = (df['IBM Gates'] - df['Our Gates']) / df['IBM Gates'] * 100
            reduction_col = 'Reduction'
