"""
Final fixed training data generation
"""
import numpy as np
import pandas as pd
import pickle
...
def generate_training_sample(n_qubits, circuit_generator, gradient_calculator):
    """Generate a single training sample with fixed 3 params per qubit"""
    try:
        # Generate random circuit (but we only use its structure, not parameters)
        ...
        # Find optimal initialization - now returns exactly 3 params per qubit
        ...
        sample = {
            ...
            'optimal_params': np.array(optimal_params, dtype=np.float32),  # Shape: [n_qubits * 3]
...
def generate_final_dataset(n_circuits=10000, max_qubits=100, min_qubits=5, 
                          output_path="data/processed/gnn_training_dataset_final.pkl"):
    ...
    dataset = {
        ...
        'metadata': {
            ...
            'avg_improvement': np.mean([s['gradient_stats']['improvement_factor'] for s in samples])
        }
    }
    ...
    metadata_path = output_dir / "dataset_metadata_final.json"
    ...
    print(f"\nPerformance metrics:")
    print(f"  Average gradient norm: {dataset['metadata']['avg_gradient_norm']:.2e}")
    print(f"  Trainable fraction: {dataset['metadata']['trainable_fraction']:.1%}")
    print(f"  Average improvement vs random: {dataset['metadata']['avg_improvement']:.1f}x")
    
    return dataset

def verify_dataset(filepath):
    """Verify dataset has correct shapes"""
    ...
    errors = []
    for i, sample in enumerate(dataset['samples'][:10]):  # Check first 10
        n_qubits = sample['n_qubits']
        expected_params = n_qubits * 3
        actual_params = len(sample['optimal_params'])
        
        if actual_params != expected_params:
            errors.append(f"Sample {i}: Expected {expected_params} params, got {actual_params}")
        
        if sample['node_features'].shape != (n_qubits, 5):
            errors.append(f"Sample {i}: Node features shape {sample['node_features'].shape}, expected ({n_qubits}, 5)")
    ...
    if errors:
        ...
        print("All samples have correct shapes! ✓")
    
    return len(errors) == 0

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Generate final GNN training data")
    parser.add_argument("--n_circuits", type=int, default=1000,
    ...
    parser.add_argument("--output", type=str, 
                       default="data/processed/gnn_training_dataset_final.pkl",
