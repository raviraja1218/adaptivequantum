"""
Fixed trainer with proper dimension handling
"""
import torch
...
class FixedGNNTrainingDataset(Dataset):
    """Dataset for GNN training with proper shapes"""
    ...
    def __getitem__(self, idx):
        ...
        # Targets: optimal parameters - reshape to [n_qubits, 3]
        # The optimal_params from generator is already flattened [n_qubits * 3]
        n_qubits = sample['n_qubits']
        optimal_params_flat = sample['optimal_params']  # Flattened
        targets = torch.FloatTensor(optimal_params_flat).view(n_qubits, 3)
...
def train_fixed_gnn(data_file, model_save_path, log_path, epochs=100, batch_size=32, 
                   learning_rate=0.001, checkpoint_dir=None):
    """Main training function for fixed GNN"""
    print("Starting fixed GNN training...")
    ...
    dataset = FixedGNNTrainingDataset(data_file)
    ...
    print(f"  Should be [n_qubits, 3]: [{sample['n_qubits']}, 3]")
    ...
    plt.tight_layout()
    plot_path = Path(log_path).parent / "training_curves_fixed.png"
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Train fixed GNN for parameter initialization")
    ...
    parser.add_argument("--log_path", type=str,
                       default="logs/thrust1/training/gnn_training_fixed.json",
